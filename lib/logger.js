...
class Logger {
   ...
   * @return {EventEmitter} Winston event emitter object
   */
  ...
  execute(log) {
    ...
    log.xRequestId = this.res._header['x-request-id'];
    const logger = new winston.Logger(this.winstonConfig)[this.winstonConfig.level](log);
  }
